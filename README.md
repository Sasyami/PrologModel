...
Для экономии VRAM можно включить квантованную загрузку train-модели и embedding-моделей через `bitsandbytes`:

```powershell
MODEL_LOAD_IN_4BIT=1
MODEL_DEVICE_MAP=auto
```

Если нужен более консервативный режим по качеству, можно вместо этого включить `MODEL_LOAD_IN_8BIT=1`. Одновременно `MODEL_LOAD_IN_4BIT` и `MODEL_LOAD_IN_8BIT` включать нельзя.
